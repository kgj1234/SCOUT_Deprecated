function BatchEndoscopeWrapper(base_dir,vids_per_batch,overlap_per_batch,num_ext_per_batch,overlap_per_file)
...
if length(batch_sizes)<=3&&vids_per_batch>num_ext_per_batch
    for j=1:length(batch_sizes)
        %try    
            BatchEndoscopeAutoHPC_adjustedgraphsearch(batch_vids{j},base_dir,batch_sizes{j},overlap_per_file,num_ext_per_batch,j);
        %catch ME
        %    ME.message
        %    ME.stack   
        %    BatchEndoscopeAutoHPC_adjustedgraphsearch(batch_vids{j},base_dir,batch_sizes{j},overlap_per_file,num_ext_per_batch,j);
